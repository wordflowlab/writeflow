 ...
 * - GLM-Z1 系列
 */
export class ZhipuStreamAdapter extends StreamAdapter {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith('data: ')) {
        try {
          ...
          const response: ZhipuStreamResponse = JSON.parse(jsonStr)
      ...
      if (data.startsWith('data: ')) {
        ...
        if (response.error) {
          return {
            ...
            error: response.error.message || 'Zhipu AI API Error',
            raw: response
          }
        }
      }
 ...
 * 便捷工厂函数
 */
export function createZhipuStreamAdapter(config?: Partial<StreamAdapterConfig>): ZhipuStreamAdapter {
