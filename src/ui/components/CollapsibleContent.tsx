 ...
 * 用于不需要复杂交互的场景
 */
export function SimpleCollapsibleContent({
  content,
  maxLines = 10,
  title
}: {
  ...
  const lines = content.split('\n')
  const shouldCollapse = lines.length > (maxLines || 5)
  const [collapsed, setCollapsed] = useState(shouldCollapse)
  
  if (!shouldCollapse) {
    return <RichTextRenderer content={content} wrap={true} preserveWhitespace={true} />
  }
